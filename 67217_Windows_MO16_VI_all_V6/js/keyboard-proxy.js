...
KeyboardProxy.prototype.enterEvent = function(){
	 ...
	 if(!isUnder(salva_pathSalva.down('1').down('type_window').down(salva_typeWindow).down(salva_typeWindow).down('save')) || !isUnder(salva_pathSalva.down('1').down('movie_apri').down(salva_typeWindow).down('apri').down('apri'))){
...
* 
*/
KeyboardProxy.prototype.f12Event = function(pEvent){
	 console.log('BODY KEY F12');
	 pEvent.SOURCE_EVENT.preventDefault();
	 if(!isUnder(word.barre.down('1').down('titolo').down('save'))  && !winSave_opened){
		salva_apri();
	 }
};
/**
* 
*/
KeyboardProxy.prototype.pasteEvent = function(pEvent){
	 console.log('BODY KEY SHIFT-TAB');
	 pEvent.SOURCE_EVENT.preventDefault();
	// if(condizione){
	//	tab.tabTextBox('shift-tab'); 
