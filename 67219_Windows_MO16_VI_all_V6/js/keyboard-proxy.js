...
KeyboardProxy.prototype.enterEvent = function(){
	 ...
	 if(!isUnder($('insertSymbol').get('path').down('1').down('simbolo').down('insert'))){
	 	$('insertSymbol').confirm();
	 }
};	 
/**
 * 
...
KeyboardProxy.prototype.ctrlAltCEvent = function(pEvent){
	 console.log('BODY KEY CTRL-ALT-C');
	 pEvent.SOURCE_EVENT.preventDefault();
	 if(!isUnder(word.barre.down('1').down('titolo').down('x'))){
		sim.addSymbolShortcut();
	 }
};
/**
* 
