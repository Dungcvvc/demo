...
function salva_apri(){
		...
		salva_specializedApri();
		winSave_opened = true;
		window.getSelection().empty();
	}

function salva_specializedApri(){
...
function salva_clickCartella(car){
			...
			if(!salva_onlySave){
				...
				if(getValue('salva_itemSalva').indexOf("file") > -1){
					salva_nomeFile_open.setText(salva_pathSalva.down('0').down(getValue('salva_itemSalva')).getText());
					car.up().up().up().up().down('movie_apri').down(salva_typeWindow).down('file').goTo('1');
				}
			}			
	}
...
function salva_openPopupFolder(position){
		...
		var item = getValue('salva_itemSalva');
		
		if(position){
			if(item.indexOf("file") > -1){
				$('dxFileSave').open(arguments[1], arguments[2]);
			}
			else{
...
function salva_back(btn){
			if(!salva_onlySave){
				...
				if(getValue('salva_cartellaSalva') != salva_FolderImage){
...
function salva_resetta(mov)
{
	...
	salva_pathSalva.down('1').down('type_window').down('open').down('nomeFile').setText('');
}
...
casella=casella.replace(/jpeg/g,'jpg');
var stringaUscita = '';
var cartella = getValue('salva_cartellaSalva');
 ...
 switch (uscita)
        {case 1:
         ...
         case 2:
				if(salva_typeWindow == 'save'){
					 stringaUscita = modo+'_/_'+cartella+'|'+casella+'|'+textForImageActivate;
				}
				else{
					 stringaUscita = modo+'_/_'+cartella+'|'+casella+'|'+textForImageActivate;
				}
...
api.addAction(''+stringaUscita.replace(/ /g, '').toLowerCase());
		 switch (stringaUscita.replace(/ /g, '').toLowerCase())
			{
/*
			case 'salva_/_root|c:\\data\\projects\\'+salva_strUscita+'_/_'+salva_tipoFileText.getText().replace(/ /g, '').toLowerCase():
			case 'salva_/_cartella1|c:\\data\\projects\\'+salva_strUscita+'_/_'+salva_tipoFileText.getText().replace(/ /g, '').toLowerCase():
			case 'salva_/_cartella2|c:\\data\\projects\\'+salva_strUscita+'_/_'+salva_tipoFileText.getText().replace(/ /g, '').toLowerCase():
			case 'salva_/_cartella2|'+salva_strUscita+'_/_'+salva_tipoFileText.getText().replace(/ /g, '').toLowerCase():
			case 'salva_/_root|projects\\'+salva_strUscita+'_/_'+salva_tipoFileText.getText().replace(/ /g, '').toLowerCase():
			case 'salva_/_root|projects\\'+salva_strUscita+'.docx_/_'+salva_tipoFileText.getText().replace(/ /g, '').toLowerCase():
			case 'salva_/_root|c:\\data\\projects\\'+salva_strUscita+'.docx_/_'+salva_tipoFileText.getText().replace(/ /g, '').toLowerCase():
			case 'salva_/_cartella1|c:\\data\\projects\\'+salva_strUscita+'.docx_/_'+salva_tipoFileText.getText().replace(/ /g, '').toLowerCase():
			case 'salva_/_cartella2|c:\\data\\projects\\'+salva_strUscita+'.docx_/_'+salva_tipoFileText.getText().replace(/ /g, '').toLowerCase():
			case 'salva_/_cartella2|'+salva_strUscita+'.docx_/_'+salva_tipoFileText.getText().replace(/ /g, '').toLowerCase():
*/
			case 'salva_/_cartella1|file2':
			case 'salva_/_cartella1|'+salva_strUscita+'.pptx':
...
function salva_setTipo(btn){
		...
		switch(btn.getName()){
			...
			break;
			case '14':
				salva_tipoFileText.setText('Works6 - 9 Document (*.wps)');
				salva_setEstensione('wps');
				salva_pathSalva.down('1').down('otherView').down('1').down('web').goTo('0') ;
			break;
		}
...
//	pathPopup.down('1').down('dxFolderSave').down('1').down('b').down('b').addAction("click","salva_back(salva_pathSalva.down('1').down('movie_cartelle').down(getValue('salva_cartellaSalva')).down('back').down('b'));");
	pathPopup.down('1').down('dxFolderSave').down('1').down('dxFolderSave').down('l').setAction("click","$('dxFolderSave').close(); salva_openFolderFromPopup();");
	pathPopup.down('1').down('dxFolderSave').down('1').down('dxFolderSave').down('o').setAction("click","$('dxFolderSave').close(); salva_openFolderFromPopup();");

	pathPopup.down('1').down('dxFileSave').down('1').down('dxFileSave').down('l').setAction("click","$('dxFileSave').close(); salva_openFileFromPopup();");
}
...
function salva_openFileFromPopup(){
		salva_doubleClickCartella(salva_pathSalva.down('1').down('movie_cartelle').down('cartella1').down('contenitore').down(getValue('salva_itemSalva')));
}
