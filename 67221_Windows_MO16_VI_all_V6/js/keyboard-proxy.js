...
KeyboardProxy.prototype.enterEvent = function(){
	 ...
	 if(!isUnder($('font').get('path').down('1').down('carattere').down('ok'))){
	 	$('font').confirm(); 
	 }
};	 
/**
...
* 
*/
KeyboardProxy.prototype.ctrlBEvent = function(pEvent){
	 console.log('BODY KEY SHIFT-TAB');
	 pEvent.SOURCE_EVENT.preventDefault();
...
* 
*/
KeyboardProxy.prototype.ctrlDEvent = function(pEvent){
	 console.log('BODY KEY CTRL-D');
	 pEvent.SOURCE_EVENT.preventDefault();
	if(!isUnder(word.barre.down('1').down('titolo').down('icona'))){
		sim.apriCarattere();
	 }
