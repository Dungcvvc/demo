...
KeyboardProxy.prototype.shortcutManagerDown = function(strShortcut, pEvent){
		...
		switch(strShortcut) {
			...
			case 'SHIFT-TAB':
				...
				break;					
			case 'CTRL-F6':
				this.ctrlF6Event(pEvent);
				break;	
			case 'CTRL-C':	
			case 'CTRL-INS':
				this.copyEvent(pEvent);
				break;
			case 'CTRL-V':	
			case 'SHIFT-INS':
...
* 
*/
KeyboardProxy.prototype.ctrlF6Event = function(pEvent){
	 console.log('BODY KEY CTRL-F6');
	 pEvent.SOURCE_EVENT.preventDefault();
	if(!isUnder(word.barre.down('1').down('titolo').down('icona'))){
		sim.switchDocument(); 
	 }
};
/**
...
KeyboardProxy.prototype.copyEvent = function(pEvent){
	 console.log('BODY KEY COPY');
	 pEvent.SOURCE_EVENT.preventDefault();
	if(!isUnder(word.barre.down('1').down('titolo').down('icona'))){
		sim.copy(); 
	 }
